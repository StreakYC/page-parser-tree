...
test('listen', async () => {
  ...
  const divIgnoreA = Object.assign(document.createElement('div'), {
    className: 'aa ignore'
  });
  const divB = Object.assign(document.createElement('div'), { className: 'b' });
  const divIgnoreB = Object.assign(document.createElement('div'), {
    className: 'bb ignore'
  });
  ...
  const parents = [];
  const liveSet = watchFilteredChildren(
    LiveSet.constant(new Set([{ el: div, parents }])),
    createCssFn('div:not(.ignore)')
  );
  ...
  expect(
    Array.from(liveSet.values()).map(({ el }) => tagAndClassName(el))
  ).toEqual([divA, divB].map(tagAndClassName));
  expect(
    next.mock.calls.map(([changes]) =>
      changes.map(({ type, value: { el } }) => [type, tagAndClassName(el)])
    )
  ).toEqual([[['add', 'div.b']]]);
});
...
test('listen, pullChanges', async () => {
  ...
  expect(
    Array.from(liveSet.values()).map(({ el }) => tagAndClassName(el))
  ).toEqual([divA].map(tagAndClassName));
...
test('do not reprocess nodes removed and re-added immediately', async () => {
  ...
  ).toEqual([[['add', 'div.b'], ['add', 'div.c'], ['remove', 'div.c']]]);
});
